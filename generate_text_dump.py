...
def main(args):
    ...
    if "mbart" in args.model_name.lower() or "bart" in args.model_name.lower():
        ...
        # +++ 🌟 新增：挂载探针和 Foresight 价值网络 +++
        print("Mounting Hidden State Extractor and Value Head...")
        extractor = HiddenStateExtractor(model)
        # max_error_dim=64 是我们策略二中设定的最大兼容长度
        value_head = WatermarkValueHead(hidden_dim=4096, max_error_dim=64, vocab_size=model.config.vocab_size).to(device)
        
        # 尝试加载训练好的权重（如果是对比实验，可以控制是否加载）
        try:
            value_head.load_state_dict(torch.load("foresight_value_head.pt"))
            value_head.eval()
            print("Successfully loaded foresight_value_head.pt!")
        except FileNotFoundError:
            print("Warning: foresight_value_head.pt not found. Running in baseline OmniMark mode.")
            value_head = None
    ...
    prompt_idx, prompts, human_written, num_test = load_data(
            ...
            args.prompt_len, 
            args.num_test,
            ds_start_point=args.ds_start_point  # 完美避开前5000 条训练数据！
        )
    ...
    for batch in range(n_batches):
        ...
        if args.method.lower() == 'bimark':
            ...
            watermark_processor_omnimark = WatermarkOmniMark(
                ...
                extractor=extractor,          # 注入探针
                value_head=value_head,        # 注入大脑
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ds_start_point", type=int, default=5000, help="dataset start index to avoid overlap with training data.")
