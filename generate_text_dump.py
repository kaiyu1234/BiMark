...
def main(args):
    ...
    auth_token = "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"
    ...
    for batch in range(n_batches):
        ...
        if args.method.lower() == 'bimark':
            ...
            if args.random_message:
                bits = "".join([str(random.randint(0, 1)) for _ in range(len(args.message))])
            else:
                bits = args.message
            ...
            watermark_processor_omnimark = WatermarkOmniMark(
                ...
                c_key=args.c_key, delta=args.prob_delta, window_size=args.window_size, bits=bits
            )
