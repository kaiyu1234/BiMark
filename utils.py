...
def load_data(dataset_name, prompt_len=100, num_test=10000, ds_start_point=0, sliding_prompt=0, model_name=None):
    if dataset_name.lower() == 'c4':
        ...
        dataset = load_dataset(
            "json", 
            data_files="hf://datasets/allenai/c4/realnewslike/c4-validation.00000-of-00001.json.gz", 
            split="train",  # JSON 加载器默认会将数据放在 train split 中
            streaming=True
        ).shuffle(seed=42)
        ...
        while t < num_test:
            ...
            # 取消这行: truncated_article = " ".join(article_text.split()[:max_article_words])
            text = article_text  # 直接传入完整文章
            ...
            # text = f"Summarize the following article concisely:\n{truncated_article}\n\nSummary:"
            prompts.append(text)
            prompt_idx.append(t)
        ...
        while t < num_test:
            ...
            text = example['translation']['ro']
            prompts.append(text)
            prompt_idx.append(t)
            # 真实的罗语翻译作为 Reference
            human_written.append(example['translation']['en'])
            t += 1
        true_num_test = num_test
