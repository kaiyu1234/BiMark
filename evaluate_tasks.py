...
def main(args):
    ...
    if args.task == 'summarization':
        ...
        print("=== 检查文本格式 ===")
        print("PRED:", repr(preds_joined[0]))  # 用 repr() 能看到隐藏的换行符和空格
        print("REF :", repr(refs_joined[0]))
        print("====================")
        print("正在计算 ROUGE 分数...")
        rouge = evaluate.load('rouge')
        rouge_results = rouge.compute(predictions=preds_joined, references=refs_joined, use_stemmer=True, use_aggregator=True)
        ...
        refs_sacrebleu = [[r] for r in refs]
        results = sacrebleu.compute(predictions=preds, references=refs_sacrebleu)
        print("\n=== 翻译任务评估结果 (SacreBLEU) ===")
